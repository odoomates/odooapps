...
class AlipayController(http.Controller):
    ...
    def _alipay_validate_data(self, **post):
        ...
        if resp:
            if resp in ['TRADE_FINISHED', 'TRADE_SUCCESS']:
                ...
                _logger.warning('Alipay: unrecognized alipay answer, received %s instead of TRADE_FINISHED/TRADE_'
                                'SUCCESS and TRADE_CLOSED' % (post['trade_status']))
        if post.get('out_trade_no') and post.get('trade_no'):
            post['reference'] = request.env['payment.transaction'].sudo().\
                search([('reference', '=', post['out_trade_no'])]).reference
    ...
    def _alipay_validate_notification(self, **post):
        if post.get('out_trade_no'):
            alipay = request.env['payment.transaction'].sudo().\
                search([('reference', '=', post.get('out_trade_no'))]).acquirer_id
        else:
        ...
        # After program is executed, the page must print “success” (without quote).
        # If not, Alipay server would keep re-sending notification, until over 24 hour 22 minutes Generally,
        # there are 8 notifications within 25 hours (Frequency: 2m,10m,15m,1h,2h,6h,15h)
